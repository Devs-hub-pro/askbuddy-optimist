...
Page({
  data: {
    ...
    overseasRegions: {
      '亚洲': ['日本', '韩国', '新加坡', '马来西亚', '泰国', '越南', '印度尼西亚', '菲律宾', '印度', '阿联酋', '以色列'],
      '欧洲': ['英国', '法国', '德国', '意大利', '西班牙', '葡萄牙', '瑞士', '瑞典', '挪威', '芬兰', '丹麦', '荷兰', '比利时'],
      '北美洲': ['美国', '加拿大', '墨西哥', '古巴'],
    ...
    hotCities: ['北京', '上海', '广州', '深圳', '杭州', '成都', '西安', '南京', '武汉', '宁波', '天津', '重庆', '日本', '美国', '英国'],
    scrollIntoView: '',
    currentCity: '',
    recentCities: [],
    searchKeyword: '',
    filteredCities: [],
    activeTab: 'mainland'  // 'mainland' or 'overseas'
  },
  ...
  searchCities: function(keyword) {
    ...
    // 添加海外地区
    Object.values(this.data.overseasRegions).forEach(countries => {
      allCityList = allCityList.concat(countries);
    });
