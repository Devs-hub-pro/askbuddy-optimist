...
const ProductFeedback = () => {
  ...
  const [lastSubmittedType, setLastSubmittedType] = useState<string | null>(null);
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="rounded-2xl bg-slate-50 px-3 py-3">
                <p className="text-xs text-slate-500">建议补充使用场景、问题现象和期望效果，便于我们更快定位和处理。</p>
              </div>
          ...
          <Card className="surface-card rounded-3xl border-none shadow-sm">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-slate-900">最近已提交反馈</p>
                  <p className="text-xs text-slate-500">类型：{types.find((item) => item.id === lastSubmittedType)?.label || '其他'}，我们会尽快查看。</p>
                </div>
              </div>
            </CardContent>
          </Card>
